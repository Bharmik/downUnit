    ...
    <el-main>
      <el-form label-position="left">
        <el-form-item label="偏爱的视频格式:">
          <el-select placeholder="请选择你偏好的视频格式"
            @change="onChange('format', $event)" :value="customSettings.format">
            <el-option label="MPEG2-TS" value="ts"></el-option>
            <el-option label="MP4" value="mp4">
              <span style="float: left">MP4</span>
              <span
                :style="{ paddingLeft: '13px', float: 'right', color: 'red', fontSize: '12px' }">
                如果原视频格式不是MP4,转换MP4有兼容性问题,慎用</span>
            </el-option>
          </el-select>
          <el-tooltip class="item" effect="dark" content="探测到的视频将优先展示此格式"
            placement="top-start">
            <i class="el-icon-question"></i>
          </el-tooltip>
        </el-form-item>
        <el-form-item label="偏爱的MP4转化器:">
          <el-select placeholder="请选择你MP4转化器"
            @change="onChange('converter', $event)"
            :value="customSettings.converter">
            <el-option label="mux.js" value="mux.js">
              <span style="float: left">mux.js</span>
              <span
                :style="{ paddingLeft: '13px', float: 'right', color: 'red', fontSize: '12px' }">转化后长宽正确,时间不正确</span>
            </el-option>
            <el-option label="mpegts-to-mp4" value="mpegts-to-mp4">
              <span style="float: left">mpegts-to-mp4 魔改版</span>
              <span
                :style="{ paddingLeft: '13px', float: 'right', color: 'red', fontSize: '12px' }">转换速度较慢</span>
            </el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="偏爱的清晰度:">
          <el-select placeholder="请选择你偏好的清晰度"
            @change="onChange('quality', $event)"
            :value="customSettings.quality">
            <el-option v-for="(item, key) in qualityMap" :key="key"
              :label="item" :value="key"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="视频过期时间:">
          <el-input-number controls-position="right"
            :value="customSettings.expiredAt"
            @change="onChange('expiredAt', $event)" :min="5" :max="240">
          </el-input-number>
          <!-- <el-tooltip class="item" effect="dark" content="单位为分钟,从采集时间算起,超过这个时间的数据将会被删除(真正删除可能有一定延迟)" placement="top-start"> -->
          <!-- <i class="el-icon-question"></i> -->
          </el-tooltip>
        </el-form-item>
        <el-form-item label="高级探测:">
          <el-switch v-model="advancedSniffer" :width="50"> </el-switch>
          <el-tooltip class="item" effect="dark" content="开启此模式后可以探测其他网站的资源"
            placement="top-start">
            <i class="el-icon-question"></i>
          </el-tooltip>
        </el-form-item>
        <template v-if="advancedSniffer">
          <el-form-item label="JSON配置:">
            <el-input type="textarea" placeholder="请输入高级探测配置"
